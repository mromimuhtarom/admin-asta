...
@section('page')
  <li class="breadcrumb-item menunameheader"><a href="{{ route('Item_Store_Reseller') }}">{{ translate_menu('L_RESELLER')}}</a></li>
  <li class="breadcrumb-item menunameheader"><a href="{{ route('Item_Store_Reseller') }}">{{ translate_menu('L_ITEM_STORE_RESELLER')}}</a></li>
@endsection
          ...
          <div class="col-9 col-sm-5 col-md-5 col-lg-5">
            <div class="input-group">
              ...
              <button class="btn sa-btn-primary" data-toggle="modal" data-target="#createGoldStore">
                <i class="fa fa-plus"></i> {{ TranslateReseller('L_MAKE_NEW') }} {{ translate_menu('L_ITEM_STORE_RESELLER')}}
              </button>
              @endif
            </div>
          </div>
          ...
          <table class="table table-bordered">
            <thead>
              <tr>
                ...
                @endif
                <th class="th-sm">{{ TranslateMenuToko('L_ORDER')}}</th>
                <th style="width:10px;">{{ TranslateMenuToko('L_IMAGE')}}</th>
                <th class="th-sm">{{ TranslateMenuToko('L_TITLE')}}</th>
                <th class="th-sm">{{ TranslateMenuToko('L_ITEM_AWARDED')}}</th>
                <th class="th-sm">{{ TranslateReseller('L_ITEM_BONUS')}}</th>
                <th class="th-sm">{{ TranslateReseller('L_ITEM_BONUS_IMAGE')}}</th>
                <th class="th-sm">{{ TranslateReseller('L_ITEM_BONUS_GET')}}</th>
                <th class="th-sm">{{ TranslateMenuToko('L_PRICE_CASH')}}</th>
                <th class="th-sm">{{ TranslateMenuToko('L_ITEM_TYPE')}}</th>
                <th class="th-sm">{{ TranslateMenuToko('L_TRANSACTION')}}</th>
                <th class="th-sm">Google Key</th>
                <th class="th-sm">{{ TranslateMenuItem('L_STATUS')}}</th>
                @if($menu && $mainmenu)
                  <th>{{ TranslateMenuToko('L_ACTION')}} &nbsp; &nbsp;
              ...
              @if($menu && $mainmenu)
                @if($gold->status === 0)
                  <tr>
                      ...
                      <div class="media-container">
                          <form method="POST" action="{{ route('ItemStoreReseller-updateimage') }}" enctype="multipart/form-data">
                            ...
                            <figure class="media-object">
                              ...
                              <img src="{{ route('imageStoRess', $gold->item_id )}}?{{ $timenow }}" class="img-object-normal imgupload{{ $gold->item_id }}" style="margin-left: auto; margin-right: auto;">
                    ...
                    <td>
                      <div class="media-container" align="center">
                        <form method="POST" action="{{ route('ItemStoreReseller-updateimageBonus') }}" enctype="multipart/form-data">
                          ...
                          <figure class="media-object">
                            <img class="img-object uploadBonusImg{{ $gold->item_id }}" src="{{ route('imageBonusStoRess', $gold->item_id) }}?{{ $timenow }}" style="margin-left: auto; margin-right: auto;">
                            <img class="img-object-wtr1Ava uploadBonusImg1{{ $gold->item_id }}" src="http://placehold.jp/80x100.png">
                          </figure>
                         
                        </div>
                        <div class="media-control" align="center" style="margin-top:-1%">
                          <button class="save-ImgBonus{{ $gold->item_id }} btn btn-primary"><i class="fa fa-save"></i>{{ Translate_menuPlayers('L_SAVE') }}</button>
                        </form>
                          <button class="cancel-ImgBonus{{ $gold->item_id }} btn sa-btn-danger"><i class="fa fa-remove"></i>{{ TranslateMenuItem('L_CANCEL') }}</button>
                          <button class="edit-ImgBonus{{ $gold->item_id }} btn btn-primary"><i class="fa fa-edit"></i>{{ Translate_menuPlayers('L_EDIT') }}</button>
                      </div>
                    </td>
                    ...
                    <td>{{ translate_menuPlayers('L_GOLD_COINS')}}</td>
                ...
                @else 
                  <tr>
                    ...
                    <td>
                      <div class="media-container">
                          <figure class="media-object">
                            <img class="img-object-normal imgupload{{ $gold->item_id }}" src="{{ route('imageStoRess', $gold->item_id) }}?{{ $timenow }}" style="  display: block;margin-left: auto;margin-right: auto;">
                          </figure>
                      </div>
                    </td>
                    <td>{{ $gold->name }}</td>
    ...
    <div class="modal-content">
      ...
      <form action="{{ route('ItemStoreReseller-create') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            <input type="number" name="goldAwarded" class="form-control" id="basic-url" placeholder="Item awarded" min="0">
          </div>
          <div class="form-group">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form action="{{ route('ItemStore-deleteAllSelected') }}" method="post">
          {{ method_field('delete')}}
          {{ csrf_field() }}
